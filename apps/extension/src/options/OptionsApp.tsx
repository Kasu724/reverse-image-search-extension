...
export function OptionsApp() {
  ...
  useEffect(() => {
    void readConverterSettings()
      .then((nextConverterSettings) => {
  ...
  async function openImageLabPanel() {
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Could not open ImageLab.");
    }
  }
    ...
    <main className="min-h-screen bg-ink-50 p-4 text-ink-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto grid max-w-4xl gap-4">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-ink-500 dark:text-slate-400">
              Configure local conversion and privacy-safe browser storage.
            </p>
          </div>
          <button
        ...
        <section className="it-panel rounded-lg p-4">
          ...
          <div className="grid gap-3">
            <p className="rounded-md bg-ink-50 p-3 text-sm text-ink-600 dark:bg-slate-800 dark:text-slate-300">
              ImageLab keeps images, notes, history, and settings on this device. External search and cloud processing are inactive.
            </p>
          </div>
        </section>
